        ...
        for param, weight_decay, beta, param_group in params():
            ...
            if param_group['add_noise']:
                ...
                if beta > 0:
                    ...
                    # When beta = 0, no noise is added (pure gradient descent)
                    if param.data.is_cuda:
                        langevin_noise = torch.cuda.FloatTensor(param.data.size()).normal_(mean=0, std=1)
                    else:
                        langevin_noise = param.data.new(param.data.size()).normal_(mean=0, std=1)
                    param.data.add_(langevin_noise, alpha=-param_group['lr'])
