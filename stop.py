...
def noise(noisetype,noiselevel):
    if noisetype == 0:
        return random.gauss(0,math.sqrt(noiselevel))
    if noisetype == 1:
...
mean, mean2, tmix = 0, 0, 0
meanstart, meanstart2 = 0,0
t    = 0
vs   = []
emas = []
em2s = []
...
while t < 20000:
    ...
    if stopped == 0:
        meanstart = meanstart + v
        meanstart2 = meanstart2 + v**2
    if stopped and t < 2*tmix:
        mean = mean + v
        mean2 = mean2 + v**2
    wbtw,lastbtw = btw (v )
    wema,lastema = ema (v )
...
mean2 = mean2 / tmix
var = mean2 - mean**2
meanstart = meanstart / tmix
meanstart2 = meanstart2 / tmix
varstart = meanstart2 - meanstart**2
print ('tmix, mean, meanstart = ',tmix,' ', mean,' ', meanstart)
print ('var, varstart = ', var,' ',varstart)

fig, ax = plt.subplots()             # Create a 
ax.plot(ts, vs, color = '#909090', linewidth=1.0, label = 'signal')                 # Plot some data on the Axes.
#ax.plot(ts, btws, color = 'g', linewidth=1.0, label = 'btw')                 # Plot some data on the Axes.
...
ax.set_ylim ([-5,100])                # Plot some data on the Axes.
ax.legend()
